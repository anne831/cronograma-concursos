    ...
    <div style={{ width: '100%' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 12 }}>
        {[
          ...
          { label: 'Horas semana', val: horasSemana.toFixed(1) + 'h', icon: '📈', color: 'var(--green)', action: 'grade' },
          ...
          { label: 'Tópicos concluídos', val: pctTopicos + '%', icon: '✏️', color: 'var(--blue)', action: 'topicos' },
        ].map((c, i) => (
          <div key={i} onClick={() => onNavigate(c.action)} style={{
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, fontFamily: 'var(--font-display)', color: c.color, lineHeight: 1 }}>
                {c.val}
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: proximaProva ? '1fr 2fr' : '1fr', gap: 12, marginBottom: 12 }}>
        ...
        {proximaProva && (
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)', marginBottom: 2 }}>{proximaProva.nome}</div>
            {proximaProva.cargo && <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 8 }}>{proximaProva.cargo}</div>}
            <div style={{
              ...
              {proximaProva.dias} <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text3)' }}>dias</span>
            </div>
          </div>
        )}
            ...
            {ultimos7.map((d, i) => (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                ...
                <div style={{
                  ...
                  height: Math.max(d.horas / maxHoras * 45, d.horas > 0 ? 4 : 2),
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 12 }}>
        ...
        <div style={{
          ...
          {materias.length === 0 ? (
            <div style={{ fontSize: 13, color: 'var(--text3)' }}>Nenhum tópico cadastrado</div>
          ) : materias.map((m, i) => (
            <div key={i} style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                <span style={{ color: 'var(--text2)' }}>{m.mat}</span>
                <span style={{ color: CORES[i % CORES.length], fontWeight: 600 }}>{m.pct}%</span>
              </div>
              <div style={{ height: 5, background: 'var(--bg3)', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ width: m.pct + '%', height: '100%', background: CORES[i % CORES.length], borderRadius: 3, transition: 'width 0.5s' }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{
          ...
          {sessoesHoje.length === 0 ? (
            <div onClick={() => onNavigate('grade')} style={{ cursor: 'pointer' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--accent2)', marginTop: 6 }}>+ Adicionar →</div>
            </div>
          ) : sessoesHoje.map(s => (
            <div key={s.id} style={{
              ...
              <span style={{ color: 'var(--text2)' }}>{s.materia}</span>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {[
              ...
              { icon: '✏️', label: 'Tópicos', action: 'topicos' },
